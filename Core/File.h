...
namespace CForge {
	...
	*/
	...
	public:
		...
		*/
		void write(const void* pBuffer, uint64_t ByteCount);
		...
		*/
		void read(void* pBuffer, uint64_t ByteCount);
		...
		*/
		...
		/**
		* \brief Template to read a number of elements of a specific type from the file.
		* \param[out] pBuffer Buffer to store the read data. Must be at least ElementCount * sizeof(T) bytes in size.
		* \param[in] Number of elements to read.
		*/
		template<typename T> void read(T* pBuffer, int32_t ElementCount = 1) {
			read(pBuffer, sizeof(T)*ElementCount);
		}//read

		/**
		* \brief Writes the data of a particular data type to the file.
