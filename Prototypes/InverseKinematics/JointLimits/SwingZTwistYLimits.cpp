...
namespace CForge {
	...
	SwingZTwistYLimits::SwingZTwistYLimits(const Eigen::Quaternionf LocalRestPose, const float MinSwing, const float MaxSwing, const float MinTwist, const float MaxTwist) : JointLimits("SwingZTwistYLimits") {		
	...
	SwingZTwistYLimits::~SwingZTwistYLimits(void) {

	}//Destructor

	Quaternionf SwingZTwistYLimits::constrain(const Quaternionf Rotation) {
		...
		float SwingAngle = std::clamp(SwingAA.angle() * SwingAA.axis().z(), m_MinSwing, m_MaxSwing);
		Swing = Quaternionf(AngleAxisf(SwingAngle, Vector3f::UnitZ()));
