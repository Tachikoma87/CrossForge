...
//	vec4 Colors[DirLightCount];
//	vec4 Attenuation[DirLightCount];
//	vec4 Direction[DirLightCount];
...
void main(){
	float Roughness = 0.2; //Material.Roughness; // texture(TexNormal, UV).w;
	float Metallic = 0.04; //Material.Metallic; // texture(TexAlbedo, UV).w;
	float Ao = 0.0; //Material.AO; //0.0; //texture(TexDepth, UV).w;
	...
	vec3 Albedo = TexColor.a * (vec3(1,1,1) * TexColor.rgb);
	#endif
	...
	vec3 CameraPos = vec3(0.0, 2.0, 0.0); //Camera.Position.xyz;
