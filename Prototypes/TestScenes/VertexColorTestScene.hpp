...
namespace CForge {
	...
	void vertexColorTestScene(void) {
		...
		std::string WindowTitle = "CForge - Vertex Color Test Scene";
		float FPS = 60.0f;
		...
		for (uint32_t i = 0; i < M.materialCount(); ++i) {
			M.getMaterial(i)->Color = Vector4f(1.0f, 1.0f, 1.0f, 1.0f);
			M.getMaterial(i)->TexAlbedo = "";
		}
		// and assign randome colors
		std::vector<Vector3f> VColors;
		for (uint32_t i = 0; i < M.vertexCount(); ++i) {
			Vector3f V;
			V.x() = CoreUtility::randRange<float>(0.0f, 1.0f);
			V.y() = CoreUtility::randRange<float>(0.0f, 1.0f);
			V.z() = CoreUtility::randRange<float>(0.0f, 1.0f);
