...
namespace CForge {
	...
	void surfaceSamplerTestScene(void) {
		...
		// initialize camera
		VirtualCamera Cam;
		Cam.init(Vector3f(0.0f, 10.5f, 30.0f), Vector3f::UnitY());
		...
		StaticActor Skydome;
		StaticActor Surface;
		SkeletalAnimationController MuscleManController;
		StaticActor Sphere;
		...
		SAssetIO::load("Assets/tmp/MuscleManPosed.obj", &M);
		SceneUtilities::setMeshShader(&M, 0.1f, 0.04f);
		M.computePerVertexNormals();
		Surface.init(&M);

		// don't clear M, we need the data later
		//M.clear();
		...
		// add muscle man
		SGNGeometry MuscleManSGN;
		SGNTransformation MuscleManTransformSGN;
		MuscleManTransformSGN.init(&RootSGN, Vector3f(0.0f, 10.0f, 0.0f));
		MuscleManTransformSGN.scale(Vector3f(1.0f/10.0f, 1.0f/10.0f, 1.0f/10.0f));
		MuscleManSGN.init(&MuscleManTransformSGN, &Surface);
		...
		MuscleManTransformSGN.rotationDelta(R);

		// prepare node to add markers
		SGNTransformation MarkerGroupSGN;
		MarkerGroupSGN.init(&MuscleManTransformSGN); // markers will always belong to this object for the purpose of this demo
		...
		while (!RenderWin.shutdown()) {
			...
			if (RenderWin.keyboard()->keyPressed(Keyboard::KEY_0, true)) {
				float MaxDist = 0.5f;
				std::vector<int32_t> Samples;
				SSampler.sampleEquidistant(MaxDist, 50, &Samples);
				...
				for (auto i : Samples) {
					...
					pMarkerSGN->init(&MarkerGroupSGN, M.vertex(i).cwiseProduct(MuscleManTransformSGN.scale()));
					pMarkerSGN2->init(pMarkerSGN, Vector3f::Zero(), MuscleManTransformSGN.rotation(), MuscleManTransformSGN.scale(), Vector3f::Zero(), MuscleManTransformSGN.rotationDelta());
					pMarkerGeomSGN->init(pMarkerSGN2, &Sphere, Vector3f::Zero(), Quaternionf::Identity(), Vector3f(5.0f, 5.0f, 5.0f));		
				}
			}
