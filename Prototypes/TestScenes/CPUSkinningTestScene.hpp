...
*                                                                           *
\****************************************************************************/
#ifndef __CFORGE_CPUSKINNINGTESTSCENE_HPP__
#define __CFORGE_CPUSKINNINGTESTSCENE_HPP__
...
namespace CForge {
	...
	class CPUSkinningTestScene : public ExampleSceneBase {
	public:
		CPUSkinningTestScene(void) {
			m_WindowTitle = "CrossForge Example - CPUSkinningTest";
		}//Constructor
		...
		void init(void) override {
			...
			m_SkeletalSkinning.init(&M, &m_BipedController);

			M.clear();

			// add skeletal actor to scene graph (Eric)			
			m_CesiumManTransformSGN.init(&m_RootSGN, Vector3f(0.0f, -0.0f, 0.0f));
			m_CesiumManOffsetSGN.init(&m_CesiumManTransformSGN, Vector3f(0.0f, 0.0f, 0.0f));
			...
			// create cube
			M.clear();
			PrimitiveShapeFactory::cuboid(&M, Vector3f(0.025f, 0.025f, 0.025f), Vector3i(1, 1, 1));
			M.computePerVertexNormals();
			CForgeUtility::defaultMaterial(M.getMaterial(0), CForgeUtility::PLASTIC_RED);

			m_Cube.init(&M);
			m_CubeTransformSGN.init(&m_RootSGN);
			m_CubeTransformIISGN.init(&m_CubeTransformSGN);
			m_CubeSGN.init(&m_CubeTransformIISGN, &m_Cube);

			m_CubeTransformSGN.rotation(Rot);
			//m_CubeTransformSGN.scale(Vector3f(3.0f, 3.0f, 3.0f));
		...
		void mainLoop(void)override {
			...
			/*auto Skeleton = m_BipedController.retrieveSkeleton();
			Skeleton[4]->LocalRotation = Quaternionf::Identity();*/


			static int32_t V = 0;

			Vector3f VPrime = m_SkeletalSkinning.transformVertex(V);
			
			m_CubeTransformIISGN.translation(VPrime);
			//m_CubeSGN.position(VPrime);
	...
	protected:
		...
		AdaptiveSkeletalActor m_CesiumMan;
		...
		SGNTransformation m_CubeTransformIISGN;
