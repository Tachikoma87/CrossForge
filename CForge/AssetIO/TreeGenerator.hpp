#ifndef __CFORGE_TREE_GENERATOR_H__
#define __CFORGE_TREE_GENERATOR_H__
...
class TreeGenerator {
	public:
		static void generateTrees(TreeType treeType, int numberTrees, string exportPath) {
			for (int i = 0; i < numberTrees; i++) {
				...
				// tree generation 
				GEOMETRY geometry;
				BRANCH_PROPERTIES branchProperties;
				ROT_POS rotPos;
	...
	protected:
		...
		static void generateTree(GEOMETRY& geometry, int recursionDepth, float numOfSplits, int complexity, ROT_POS rotPos, BRANCH_PROPERTIES branchProperties) {
			if (recursionDepth <= 0) {
				return;
			}
			...
			for (int i = 0; i < numOfSplits; i++) {
				extraBranchIndices.push_back(randomI(geometry.vertices.size() - (complexity * complexity * 2 / 3), geometry.vertices.size() - 1));
			}
			for (int i = 0; i < numOfSplits; i++) {
				ROT_POS extraBranchRotPos;
				extraBranchRotPos.position = geometry.vertices[extraBranchIndices[i]] - geometry.normals[extraBranchIndices[i]] * branchProperties.topRadius;
				extraBranchRotPos.rotation = Quaternionf().setFromTwoVectors(Vector3f(0, 1, 0), geometry.normals[extraBranchIndices[i]]);
				extraBranchRotPos.rotation.x() *= 0.6;
				extraBranchRotPos.rotation.z() *= 0.6;
				extraBranchRotPos.rotation.normalize();
				generateTree(geometry, recursionDepth - 1, numOfSplits, complexity, extraBranchRotPos, branchProperties);
			}
		...
		static void generateAspenBirkTree(GEOMETRY& geometry, int complexity, int numBranches, ROT_POS rotPos, BRANCH_PROPERTIES branchProperties) {
			// main trunk
		...
		static ROT_POS generateBranch(GEOMETRY& geometry, ROT_POS rotPos, int numCircles, int pointsPerCirlce, BRANCH_PROPERTIES branchProperties) {
			...
			float uvStepVertically = stepDistance / (2 * PI * radius); // Textur Verzerrung vermeiden indem der Kreisumfang auf 1 normiert wird
			float uvCordU = 0;
